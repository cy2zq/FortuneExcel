...
UDOC.G = {
  ...
  // a inside b
  insideBox: function (a: any, b: any) {
  ...
  boxArea: function (a: any) {
    var w = a[2] - a[0],
      h = a[3] - a[1];
    return w * h;
  },
  newPath: function (gst: any) {
    gst.pth = { cmds: [], crds: [] };
  },
  moveTo: function (gst: any, x: any, y: any) {
    var p = UDOC.M.multPoint(gst.ctm, [x, y]); //if(gst.cpos[0]==p[0] && gst.cpos[1]==p[1]) return;
    gst.pth.cmds.push("M");
    gst.pth.crds.push(p[0], p[1]);
    gst.cpos = p;
  },
  lineTo: function (gst: any, x: any, y: any) {
  ...
  polyClip: function (p0: any, p1: any) {
    // p0 clipped by p1
...
UDOC.M = {
  getScale: function (m: any) {
    return Math.sqrt(Math.abs(m[0] * m[3] - m[1] * m[2]));
  },
  translate: function (m: any, x: any, y: any) {
    UDOC.M.concat(m, [1, 0, 0, 1, x, y]);
  },
  rotate: function (m: any, a: any) {
    UDOC.M.concat(m, [
      Math.cos(a),
      -Math.sin(a),
      Math.sin(a),
  ...
  scale: function (m: any, x: any, y: any) {
  ...
  multPoint: function (m: any, p: any) {
    var x = p[0],
      y = p[1];
    return [x * m[0] + y * m[2] + m[4], x * m[1] + y * m[3] + m[5]];
  },
  multArray: function (m: any, a: any) {
...
FromEMF._readBox = function (buff: any, off: any) {
  var b = [];
  for (var i = 0; i < 4; i++) b[i] = FromEMF.B.readInt(buff, off + i * 4);
  return b;
};
...
FromEMF._drawPoly = function (
  ...
  for (var j = 0; j < ppp; j++) {
    ...
    var py = rS(buff, off);
    off += nl;
    if (j == 0 && !justLine) UDOC.G.moveTo(gst, px, py);
    else UDOC.G.lineTo(gst, px, py);
  }
...
FromEMF.B = {
  ...
  readShort: function (buff: any, p: any): any {
    var u8 = FromEMF.B.uint8;
    u8[0] = buff[p];
    u8[1] = buff[p + 1];
    return FromEMF.B.int16[0];
  },
  readUshort: function (buff: any, p: any): any {
    var u8 = FromEMF.B.uint8;
    u8[0] = buff[p];
    u8[1] = buff[p + 1];
    return FromEMF.B.uint16[0];
  },
  readInt: function (buff: any, p: any): any {
    var u8 = FromEMF.B.uint8;
    u8[0] = buff[p];
    u8[1] = buff[p + 1];
    u8[2] = buff[p + 2];
  ...
  readUint: function (buff: any, p: any): any {
    var u8 = FromEMF.B.uint8;
    u8[0] = buff[p];
    u8[1] = buff[p + 1];
    u8[2] = buff[p + 2];
  ...
  readFloat: function (buff: any, p: any): any {
    var u8 = FromEMF.B.uint8;
    u8[0] = buff[p];
    u8[1] = buff[p + 1];
    u8[2] = buff[p + 2];
  ...
  readASCII: function (buff: any, p: any, l: any): any {
    var s = "";
...
ToContext2D.prototype.PutImage = function (
  gst: any,
  buff: any,
  w: any,
  h: any,
...
ToContext2D.prototype._colr = function (c: any, a: any) {
  return (
    "rgba(" +
    Math.round(c[0] * 255) +
    "," +
...
ToContext2D.prototype._scale = function (m: any) {
  return Math.sqrt(Math.abs(m[0] * m[3] - m[1] * m[2]));
};
